"""
CRVD 数据集 NLM (Non-Local Means) 降噪基线 - VST 版本
  - 噪声参数从 out/results/crvd_noise_params.csv 读取
  - 核心流程：前向 VST -> NLM (sigma=1.0 固定) -> 后向逆 VST
  - 输出：denoised TIFF + PSNR CSV
      → out/results/CRVD/nlm_vst/<scene>/<ISO>/
"""
...
# ── NLM 超参数 ────────────────────────────────────────────────────────────────
PATCH_SIZE    = 5    # 局部块半径（skimage 参数为 patch_kw['patch_size']）
PATCH_DIST    = 6    # 搜索窗口半径
H_FACTOR      = 1.15 # h = H_FACTOR * sigma_vst，控制滤波强度
...
def process_one(args: tuple) -> dict:
    ...
    # 2. NLM 滤波（VST 域，sigma_vst ≈ 1.0）
    sigma_vst = 1.0
    h = H_FACTOR * sigma_vst

    t0 = time.perf_counter()
    denoised_vst = denoise_nl_means(
        noisy_vst.astype(np.float64),
        h=h,
        sigma=sigma_vst,
...
def run(
    ...
    results_root: str = 'out/results/CRVD/nlm_vst',
    csv_out:      str = 'out/results/CRVD/nlm_vst/results.csv',
    ...
    with ProcessPoolExecutor(max_workers=num_workers) as executor:
        ...
        pbar = tqdm(total=len(tasks), desc='NLM-VST', dynamic_ncols=True)
        for fut in futures:
            try:
                row = rows[len(rows):] or []  # keep ref
                r = fut.result()
                rows.append(r)
                pbar.set_postfix({
                    'ISO':   r['iso'],
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='CRVD NLM 降噪基线（VST + 跨图像多进程）')
    parser.add_argument('--root',    default='data/CRVD/noisy')
    parser.add_argument('--out',     default='out/results/CRVD/nlm_vst')
    parser.add_argument('--csv',     default='out/results/CRVD/nlm_vst/results.csv')
